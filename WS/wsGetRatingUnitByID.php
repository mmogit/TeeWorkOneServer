...
function wsGetRatingUnitByID($action, $data) {
	$data = json_decode($data, true);
	$dbBasicFunctions = new dbBasicFunctions();
	$result = json_encode($dbBasicFunctions->dbSelect("SELECT * FROM ".TW_RATING_UNIT_TABLE." where id='".$dbBasicFunctions->escape_string($data['id'])."'"));
    if($result)
